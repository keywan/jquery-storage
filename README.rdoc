...
== saving objects
None of the Storages do not support saving objects (other then Strings), but you can use jQuery JSON Plugin
to build json strings.

    var cm = jQuery.cookie.getStorage(options);
    var value = $.toJSON(object);
    cm.setItem(key,value);

    var value = cm.getItem(key);
    var object = $.evalJSON(value);
    cm.removeItem(key);
