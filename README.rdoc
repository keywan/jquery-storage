...
== status
it pass all tests but i expect that there are still bugs on deleting cookies
if you like to use it in production verify that it works in all browsers as accpected.
smarter encoding Stratagie to save cookie space and allow special charaters in key and values.
...
== Usage Object API
    ...
    var cm = new $.CookieManager(options);
    cm.set(key,value);
    cm.get(key);
    cm.delete(key);
